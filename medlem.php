<head>
    <link rel="stylesheet" href="css/medlem.css" type="text/css">
    <link rel="stylesheet" href="css/bootstrap.css" type="text/css">
    <link rel="stylesheet" href="css/styles.css" type="text/css">
</head>

<?php include "includes/menu.php";?>

<div class="container">
<section class="indmeldning">
    <div class="container__frivillig">
        <div class="container py-2 px-0">

        <h1>Frivillig</h1>
        <h2 class="pt-5">Fordele</h2>

        </div>

        <div class="container__frivillig__tekst">


                    <div class="text d-flex py-4">
                        <img class="me-3" src="img/support.svg" alt="fællesskab" height="30" width="30">
                        <p>Med frivilligt arbejde har du mulighed for at lære nye mennesker at kende og indgå i nye fællesskaber.</p>
                    </div>

                    <div class="text d-flex py-4">
                        <img class="me-3" src="img/handsholdingheart.svg" alt="fællesskab">
                        <p>Du ser med egne øjne, den forskel du gør for andre mennesker gennem dit frivillige arbejde.</p>
                    </div>

                    <div class="text d-flex py-4">
                        <img class="me-3" src="img/book.svg" alt="fællesskab">
                        <p>Bliv bedre til det du allerede kan, få ny viden, erfaring og kompetencer der styrker dig som menneske og er rigtig godt på dit CV.</p>
                    </div>
                    <br><br>

                    <div class="d-flex justify-content-center pt-2 pb-5">
                        <a class="btn btn-yellow" href="#" role="button">Bliv Frivillig</a>
                    </div>

        </div>
    </div>

    <div class="container__medlem">
        <div class="container py-2 px-0">

        <h1>Medlemskab</h1>
        <h2 class="pt-5">Fordele</h2>

        </div>

        <div class="container__medlemskab__tekst">

                    <div class="text d-flex py-4">
                        <img class="me-3" src="img/heart.svg" alt="fællesskab" height="30" width="30">
                        <p>For under 3,5 kr om dagen, er du med til at hjælpe andre i nød.</p>

                    </div>

                    <div class="text d-flex py-4">
                        <img class="me-3" src="img/love.svg" alt="fællesskab">
                        <p>Det giver ro i sindet at vide, dine penge går til et godt formål.</p>

                    </div>

                    <div class="text d-flex py-4">
                        <img class="me-3" src="img/community.svg" alt="fællesskab">
                        <p>Du bliver en del af et større lokalt fællesskab der er med til at gøre en forskel.</p>
                    </div>
                        <div class="text justify-content-center">
                            <h5>99kr. / måneden</h5>
                        </div>
                    <div class="d-flex justify-content-center pt-2 pb-5">
                        <a class="btn btn-yellow" href="#" role="button">Bliv Medlem</a>
                    </div>

        </div>
    </div>

</section>
</div>

<?php include "includes/footer.php";?>




